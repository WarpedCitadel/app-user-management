...
COPY --from=builder /app/target/*.jar /app-user-management-0.0.3-SNAPSHOT.jar

RUN mkdir /deps && cd /deps && \
    jar -xf /app-user-management-0.0.3-SNAPSHOT.jar BOOT-INF/lib && \
    jdeps --ignore-missing-deps --print-module-deps --multi-release 25 \
    --class-path 'BOOT-INF/lib/*' \
    /app-user-management-0.0.3-SNAPSHOT.jar > /modules.txt
...
COPY --from=builder /app/target/*.jar /app/app-user-management-0.0.3-SNAPSHOT.jar
...
EXPOSE 8080
CMD ["java", "-jar", "/app/app-user-management-0.0.3-SNAPSHOT.jar"]
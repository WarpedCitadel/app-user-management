# app-user-management
This project contains the source code and supporting files for the applications user management mircoservice.
...
services:
  wc_local:
    image: app-user-management/local
    env_file:
      - .env
    ports:
      - "8080:8080"
...
```
Once `make` is installed, navigate to the root directory of the project via `/app-user-management` where the `Makefile` is located and enter `make readme` as a command.
```bashrc
make readme
```
